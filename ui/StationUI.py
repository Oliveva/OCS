...
class BaseStationUI(tk.Toplevel):
    def __init__(self, parent, station_id, callback, dict_data=None, show_data=None, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        self.id = station_id
        self.title(f"地面基站-{station_id}")
        self.geometry("900x750")
        self.resizable(True, True)
        self.config_dict = dict_data or {}
        self.logic = None
    ...
    def _create_config_interface(self):
        ...
        ttk.Label(ip_frame, text="当前基站IP(格式 1.0.0.1):").pack(side=tk.LEFT, padx=5)
        self.station_ip_entry = ttk.Entry(ip_frame, width=20)
        self.station_ip_entry.pack(side=tk.LEFT, padx=5)
        self.station_ip_entry.insert(tk.END,
                                     "1.0.0.1" if not self.config_dict else self.config_dict.get('ip_text', "1.0.0.1"))

        id_frame = ttk.LabelFrame(self.config_frame, text="基站ID配置", padding=10)
        id_frame.pack(fill=tk.X, pady=5)
        ttk.Label(id_frame, text="当前基站ID(格式 1:2:0):").pack(side=tk.LEFT, padx=5)
        self.station_id_entry = ttk.Entry(id_frame, width=20)
        self.station_id_entry.pack(side=tk.LEFT, padx=5)
        self.station_id_entry.insert(tk.END,
                                     "3:2:1" if not self.config_dict else self.config_dict.get('id_text', "3:2:1"))
    ...
    def _create_data_interface(self):
        ...
        self.on_data_type_selected(None)
    ...
    def save_config(self, show=True):
        ip_text = self.station_ip_entry.get()
        id_text = self.station_id_entry.get()

        station_ip = self.ip_to_int(self.station_ip_entry.get())
        if not station_ip:
            messagebox.showerror("错误", "IP格式不正确（需为10进制数.分隔）")
            return

        station_id = self.id_to_bytes(self.station_id_entry.get())
        if not station_id:
